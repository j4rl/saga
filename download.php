...
$version = $versionId > 0 ? get_project_version($conn, $versionId) : null;
$projectId = $version ? (int) $version['project_id'] : (int) ($_GET['id'] ?? 0);
...
$storedFilename = $version ? (string) $version['stored_filename'] : (string) $project['pdf_filename'];
$filePath = realpath(UPLOAD_DIR . DIRECTORY_SEPARATOR . basename($storedFilename));
...
$originalName = $version ? (string) $version['original_name'] : ($project['pdf_original_name'] ?: 'gymnasiearbete.pdf');
